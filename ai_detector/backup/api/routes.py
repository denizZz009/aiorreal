...
def analyze_image_file(file_path: str, fast_mode: bool = False):
    ...
    if watermark_result['watermark_detected']:
        engine.add_detection('watermark_detected', True, 
                           f"Watermark detected: {', '.join(watermark_result['detections'])}")
    
    # 2. Frequency Analysis
    freq_analyzer = FrequencyAnalyzer()
    freq_result = freq_analyzer.analyze(image)
    
    if freq_result['freq_ratio_anomaly']:
        engine.add_detection('freq_ratio_anomaly', True, "DCT frequency ratio anomaly")
    
    if freq_result['checkerboard_pattern']:
    ...
    if not fast_mode:
        ...
        if noise_result['noise_variance_low']:
    ...
    if color_result['rgb_correlation_high']:
    ...
    if not fast_mode:
        ...
        if geom_result['edge_fragmented']:
...
def analyze_video_file(file_path: str, fast_mode: bool = False):
    ...
    if len(frames) >= 2:
        ...
        if temporal_result['temporal_flicker']:
    ...
    if not fast_mode and len(frames) >= 2:
        ...
        if motion_result['motion_vector_irregular']:
